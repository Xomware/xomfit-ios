...
#ifndef XOMFIT_SWIFT_H
#define XOMFIT_SWIFT_H
...
# undef any
# pragma clang attribute push(__attribute__((external_source_symbol(language="Swift", defined_in="Xomfit",generated_declaration))), apply_to=any(function,enum,objc_interface,objc_category,objc_protocol))
...
SWIFT_CLASS("_TtC6Xomfit11AppDelegate")
...
/// Tokens are persisted via <code>@AppStorage</code> (UserDefaults) for v1 — readable by anyone with
/// device access. Migrating to Keychain is tracked separately and explicitly out of scope here.
/// <h2>Callback delivery</h2>
/// <code>ASWebAuthenticationSession</code> returns the redirect URL directly to our completion handler,
/// so no <code>CFBundleURLTypes</code> registration is required. The <code>XomfitApp.onOpenURL</code> handler is a
/// belt-and-suspenders fallback in case Safari View Controller hands the URL back to the app
/// instead of the session.
SWIFT_CLASS("_TtC6Xomfit18SpotifyAuthService")
@interface SpotifyAuthService : NSObject
